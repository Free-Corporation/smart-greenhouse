...
void sendhalfbyte(uint8_t c)
{
	...
	LCD_WriteByteI2CLCD((portlcd&=~0x04)|c); //E line disable
	ets_delay_us(1);
}
//------------------------------------------------------
...
void LCD_ini(void)
{
	...
	ets_delay_us(4500);
	sendhalfbyte(0x02);
	sendbyte(0x28,0); //4-bit, 2 lines 00101000
	sendbyte(0x08,0); // off display
	ets_delay_us(1000);
	sendbyte(0x01,0); //off garbage
	ets_delay_us(2000);
	sendbyte(0x06,0); //right writing
	ets_delay_us(1000);
	sendbyte(0x0C,0); //off display (D=1), -cursors
